...
pub async fn handle_error(request: Request, next: Next) -> Response {
    ...
    let is_json = content_type.map_or(false, |v| v.to_str().map_or(false, |s| s.contains("application/json")));
    ...
    let error_message = status.canonical_reason().unwrap_or("Unknown error").to_string();
